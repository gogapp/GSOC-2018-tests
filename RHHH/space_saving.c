...
/* TODO: Test the written code for correctness. */
...
int SSiterate(struct rte_hash *ct_table, int proto, int threshold)
{
	...
	uint32_t next = 0;
	ret = rte_hash_iterate(ct_table, (const void **)&key, (void **)&element, &next);
	while(ret >= 0) {
		if(proto == ETHER_TYPE_IPv4) {
			inet_ntop(AF_INET, &(element->key.k.v4.src), ip4_src, INET_ADDRSTRLEN);
			inet_ntop(AF_INET, &(element->key.k.v4.dst), ip4_dst, INET_ADDRSTRLEN);
			//printf("Src = %s Dst = %s Count = %u Err = %u\n", ip4_src, ip4_dst, element->bkt_id, element->err);
		} else if(proto == ETHER_TYPE_IPv6) {
			inet_ntop(AF_INET6, &(element->key.k.v6.src), ip6_src, INET6_ADDRSTRLEN);
			inet_ntop(AF_INET6, &(element->key.k.v6.dst), ip6_dst, INET6_ADDRSTRLEN);
		...
		if(element->bkt_id >= threshold) {
			...
			while(hh_size <= numhitter)
				hh_size = hh_size * 2;
			hh_table = (struct ip_key *)realloc(hh_table, hh_size*sizeof(struct ip_key));
	...
	for(i = 1; i <= numhitter; i++){
		if(proto == ETHER_TYPE_IPv4) {
			inet_ntop(AF_INET, &(hh_table[i].k.v4.src), ip4_src, INET_ADDRSTRLEN);
			inet_ntop(AF_INET, &(hh_table[i].k.v4.dst), ip4_dst, INET_ADDRSTRLEN);
			printf("Src = %s Dst = %s\n", ip4_src, ip4_dst);
		} else if(proto == ETHER_TYPE_IPv6) {
			inet_ntop(AF_INET6, &(hh_table[i].k.v6.src), ip6_src, INET6_ADDRSTRLEN);
			inet_ntop(AF_INET6, &(hh_table[i].k.v6.dst), ip6_dst, INET6_ADDRSTRLEN);
			printf("Src = %s Dst = %s\n", ip6_src, ip6_dst);
		}
	}
	printf("\n");
